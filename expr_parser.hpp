...
std::string fstr(const std::string& text) {
    ...
    while(i < head_size) {
        if(parser::match(i, head, "{}", false) && count < args) {
            ...
            if(/*item[0] == '$'*/ parser::is_var(item[0]) && item[1] != '&') {
...
std::string eval_indices(const std::string& text, int& index) {
    ...
    while(index < size) {
        ...
        if(/*intermediate[0] == '$'*/ parser::is_var(intermediate[0]) && intermediate[1] != '&') {
...
bool is_list_or_dict(std::string structure) {
    ...
    if(/*structure[0] == '$'*/ parser::is_var(structure[0]) && structure[1] != '&') {
...
bool is_num(std::string& data) {
    if(/*data[0] == '$'*/ parser::is_var(data[0]) && data[1] != '&') {
...
bool is_list(std::string& structure) {
    ...
    else if(/*structure[0] == '$'*/ parser::is_var(structure[0]) && structure[1] != '&') {
        Value* temp = VarTable::get(structure, {}, true, true, true);
...
std::deque<std::string> extract_operand(std::deque<std::string>& rpn) {
    ...
    if((back[0] >= '0' && back[0] <= '9') || /*back[0] == '$'*/ (parser::is_var(back[0]) && back[1] != '&') || back[0] == '"') {
        expr.push_back(back);
        rpn.pop_back();
    }
    else {
        ...
        if(!((second[0] >= '0' && second[0] <= '9') || /*second[0] == '$'*/ (parser::is_var(second[0]) && second[1] != '&') || second[0] == '"')) {
...
std::string eval(std::deque<std::string> rpn) {
    ...
    if(rpn.size() == 1 && (rpn_front[0] != '$' || rpn_front[1] != '(')) {
        ...
        else if(parser::is_var(rpn_front[0]) && rpn_front[1] != '&') {
            rpn.pop_front();
            return rpn_front;
        }
        rpn.pop_front();
